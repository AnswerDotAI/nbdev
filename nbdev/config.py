...
# %% auto 0
__all__ = ['licenses', 'statuses', 'py_versions', 'nbdev_create_config', 'get_config', 'config_key', 'cfg2pyproj', 'is_nbdev',
           'create_output', 'show_src', 'update_version', 'add_init', 'write_cells']
...
def _apply_defaults(
    ...
    pyproject_toml:bool_arg=False, # Use pyproject_toml not setup.py for package installation
):
...
# %% ../nbs/api/01_config.ipynb
statuses = ['0 - Pre-Planning', '1 - Planning', '2 - Pre-Alpha', '3 - Alpha','4 - Beta', '5 - Production/Stable', '6 - Mature', '7 - Inactive' ]
...
def cfg2pyproj(cfg):
    ...
    pyproj = {
        ...
        "project": {
            "name": cfg.lib_name,
            "version": cfg.version,
            "requires-python": f">={cfg.min_python}",
            "dependencies": ['packaging'] + cfg.requirements.split() + cfg.pip_requirements.split(),
            "authors": [{"name": cfg.author, "email": cfg.author_email}],
            ...
            ] + [f"Programming Language :: Python :: {o}" for o in py_versions[py_versions.index(cfg.min_python):]] 
              + ([f"License :: {lic[1]}"] if lic[1] else []),
            "urls": {
                "Homepage": cfg.git_url,
                "Documentation": cfg.doc_host + cfg.doc_baseurl,
                "Repository": cfg.git_url,
                "Issues": f"{cfg.git_url}/issues",
                "Changelog": f"{cfg.git_url}/{cfg.branch}/CHANGELOG.md"
            },
            "scripts": {k.strip(): v.strip() for k, v in (l.split('=', 1) for l in cfg.console_scripts.splitlines() if l.strip())},
            "entry-points": {'nbdev':{cfg.lib_name : f"{cfg.lib_name}._modidx:d"}}  
         },
        "tool": {
            "setuptools": {
                "include-package-data": True,
                "zip-safe": False,
                "packages": [cfg.lib_name]
            }
        }
    }

    # Add optional sections only if they exist
    if cfg.dev_requirements:
        pyproj['project']['optional-dependencies'] = {"dev": cfg.dev_requirements.split()}

    if dl := cfg.get('dep_links'):
        pyproject["tool"]["setuptools"]["dependency-links"] = dl.split()
