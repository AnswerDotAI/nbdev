...
# %% auto 0
__all__ = ['typs', 'bset', 'ruff_format', 'ruff_fix', 'patch_name', 'nbglob', 'nbglob_cli', 'nbdev_export', 'create_index',
           'NbdevLookup']
...
# %% ../nbs/api/05_doclinks.ipynb
def ruff_format(path=None):
    "Format code with `ruff format` on the entire library or a specific file"
    import subprocess
    try:
        cmd = ['ruff', 'format']
        if path is not None:
...
# %% ../nbs/api/05_doclinks.ipynb
def ruff_fix(path=None):
    "Lint and auto-fix code with `ruff check --fix` on the entire library or a specific file"
    import subprocess
    try:
        cmd = ['ruff', 'check', '--fix']
        if path is not None:
...
def _build_modidx(dest=None, nbs_path=None, skip_exists=False):
    ...
    cfg = get_config()
    if cfg.ruff_formatting: ruff_format(path=idxfile)
    if cfg.ruff_fixing: ruff_fix(path=idxfile)
