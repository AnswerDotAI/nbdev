...
All directive spellings are equivalent: `#| default_exp: core`,
`#| default_exp:core`, and `#| default_exp core` mean the same thing,
and a value of `true` is the same as no value, so `#| hide: true` and
`#| hide` are one directive. Directives can also be stored in cell
metadata instead of comments, as a dict under an `nbdev` key:
`{"nbdev": {"hide": "true", "eval": "false"}}` behaves exactly like
`#| hide` and `#| eval: false`. Metadata values are always str, the same
raw text a comment directive carries (`"true"` being the only special
string); JSON `true`/`false` raise an error rather than silently
differing from the string forms. Notebook-scope directives such as
`default_exp` can go in the notebook metadata the same way. When the
same directive appears in both a comment and metadata, the comment wins.
At docs render time, every directive is written into the cell source as
a Quarto option (`name: value`, bare directives as `name: true`); Quarto
applies the options it knows and ignores the rest.
...
> ### 📓 `#| exportd`
>
> A `d`ocstring export: the cell is included in the generated module’s
> docstring, and is not exported as code. Markdown cells are included
> as-is (same as `#| export` on a markdown cell), and a code cell’s
> source is included as a fenced code block, so tested example cells can
> be shown in the docstring without maintaining a separate copy. The
> docstring is built from the notebook’s `#` title cell’s `>` summary
> quote plus all such cells, in notebook order.
...
> ```
>
>     datetime.datetime(2026, 8, 21, 22, 43, 35, 739704)
...
- `skip_exec: true`: `nbdev-test` skips the notebook entirely and
  reports it as passing. Use it for notebooks that can’t run in a test
  environment at all, such as those needing credentials, running
  servers, or other live services.
- `skip_showdoc: true`: docs rendering doesn’t execute any cells in the
  notebook; the stored outputs are used as-is.
