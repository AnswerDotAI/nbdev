...
nbdev provides the `nbdev-new` command to initialise an empty git
repository. It’ll infer information about your project from git and
GitHub, and ask you to input anything remaining. It will create files in
your repo that:
...
> **Note**
>
> `nbdev-new` assumes that your package name is the same as your repo
> name (with `-` replaced by `_`). Use the `--lib_name` option if that
> isn’t the case.

</div>

Double-check your `pyproject.toml` file to ensure that it has all of the
...
- Initialised your repo with `nbdev-new`
- Pushed to GitHub.
...
You might have noticed that `nbdev-new` created a Python package in your
repo. In our case, it was automatically named `nbdev_hello_world` by
using our repo name `nbdev-hello-world` and replacing `-` with `_` to
make it a valid Python package.
...
feedback loops. The `nbdev-preview` command helps achieve this by using
Quarto to render your docs on your computer and keep them updated as
your edit your notebooks.
...
Now, open the `nbs/00_core.ipynb` file (generated by running `nbdev-new`
earlier) in Jupyter. You don’t *have* to start your notebook names with
a number, but we find it helpful to show the order that your project
should be read in – even though it could have been created in a
different order.
...
- `nbdev-export` is the command used to export your notebooks to Python
  modules.
...
This is a test too! When you run `nbdev-test` it will execute this cell
(and all other test cells) and fail if they raise any exceptions.
...
`nbdev-prepare` in the terminal, which bundles the following commands:

- `nbdev-export`: Builds the `.py` modules from Jupyter notebooks
- `nbdev-test`: Tests your notebooks
- `nbdev-clean`: Cleans your notebooks to get rid of extreanous output
  for git
- `nbdev-readme`: Updates your repo’s `README.md` file from your index
  notebook.
...
before, always remember to run `nbdev-prepare` before you commit to
...
- Installed hooks for git-friendly notebooks with `nbdev-install-hooks`
- Installed your package with `pip install -e '.[dev]'`
- Previewed your docs with `nbdev-preview`
- Added your own frontmatter, function, tests, and docs to
  `nbs/00_core.ipynb`
- Prepared your changes with `nbdev-prepare`
...
prerequisites to `[project].dependencies` in your `pyproject.toml`.
...
You can also add optional dependencies using
`[project.optional-dependencies]`. For more information about the format
of dependencies, see the [pyproject.toml
specification](https://packaging.python.org/en/latest/specifications/pyproject-toml/).

### Set up console scripts

Behind the scenes, nbdev uses the standard package `setuptools` for
...
To add one, include a `[project.scripts]` section in your
`pyproject.toml`, with one line per script. For instance, here’s the
line that creates nbdev’s own `nbdev-export` command:
...
To upload your project to pypi, just type `nbdev-pypi` in your project
...
To upload to anaconda, just type `nbdev-conda` in your project root
directory.

### Upload to pypi and conda

The command `nbdev-release-both` from the root of your nbdev repo will
