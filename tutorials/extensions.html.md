...
We just need one: the
[`Processor`](https://nbdev.fast.ai/api/process.html#processor) class
that will actually perform what we want on notebook cells. Reading `#|`
directives needs no import at all, since every notebook cell carries
them in its `directives` property.
...
def convert_layout(
    ...
    div_ = cell.directives_["div"].split()
    # By the time procs run, the pipeline has stripped directive lines from the source
    if "end" in div_: cell.source = ":::"
    else:
        if "start" in div_: div_.remove("start")
        cell.source = _LAYOUT_STR.substitute(layout=" ".join(div_), content=cell.source)
...
cell.directives_ = cell.directives  # NBProcessor sets this snapshot before procs run
```

Cells parse their own directives, available as a plain dict through the
`directives` property:
...
``` python
cell.remove_directives()  # the pipeline strips directives before procs run
convert_layout(cell)
print(cell.source)
```
...
Inside a processor, read directives from `cell.directives_` (with the
underscore), not the `directives` property: the pipeline strips
directives from cell sources before procs run, so the property would
re-parse the stripped source and come up empty. `directives_` is the
snapshot the pipeline took first.
    ...
    def cell(self, cell):
        if cell.cell_type == "markdown" and "div" in cell.directives_:
            ...
            if self.has_multiple_cells and "end" in div_:
                ...
                if is_start: self.has_multiple_cells = True
