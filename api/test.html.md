...
[`test_nb`](https://nbdev.fast.ai/api/test.html#test_nb) is async.
`parallel` workers are fresh loopless processes, so each enters through
[`asyncio.run`](https://docs.python.org/3/library/asyncio-runner.html#asyncio.run)
at its process boundary, which keeps cells on the worker’s main thread
(where SIGALRM timeouts can arm). `run_sync` is reserved for sync
callers already inside a running loop, like
[`exec_show_docs`](https://nbdev.fast.ai/api/processors.html#exec_show_docs)
under the sync processor pipeline.
...
with tempfile.TemporaryDirectory() as td:
    ...
    with modified_env(IPYTHONDIR=str(td)):
        ...
        assert not (await test_nb(nbp, showerr=False, profile=False))[0]
```
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/test.py#L92"
target="_blank" style="float:right; font-size:smaller">source</a>
