...
Each issue title will be added. Open `CHANGELOG.md` in your editor and
make any edits that you want, and then commit the file to your repo
(remember to `git add` it!)
...
If the changelog was generated and reviewed separately, complete the
release without regenerating it, opening an editor, or prompting:

    nbdev-release-gh --no-changelog --no-editor --yes

Automatic changelog generation stops when the current branch does not
contain the latest GitHub release, as happens for an update to an older
version. Write `CHANGELOG.md` manually and use the command above. The
next section shows the complete process.

See the screencast above for a demonstration of the interactive
workflow.

### Releasing an update to an older version

Sometimes the latest release is 2.x, but you need to fix version 1.x.
The example below starts from release `1.1.0` and publishes `1.1.1`.

A Git worktree is a second project folder connected to the same
repository. It lets you open the older version without changing your
usual project folder:

``` sh
git fetch origin --tags
git worktree add ../myproject-1.x -b 1.x 1.1.0
cd ../myproject-1.x
```
...
Write the new `CHANGELOG.md` entry yourself. Automatic changelog
generation is intended for the latest release, so skip it here. Review
...
The release command pushes the `1.x` branch to GitHub, then tags the
code it released. Because `1.x` is not the repository’s default branch,
the release is not marked as “Latest” on GitHub, so the repo front page
keeps showing the newest version. After checking the GitHub release and
PyPI package, remove the extra folder:

``` sh
cd ../myproject
git worktree remove ../myproject-1.x
```

The extra folder uses a little more disk space, and you need to run each
command in the right folder. In return, the latest and older versions
remain available at the same time. This is useful when other terminals
or Python sessions are using the main project folder.

For a small update with a clean project folder, you can instead switch
that folder to the older release:

``` sh
git fetch origin --tags
git switch -c 1.x 1.1.0
```

Then follow the same bump, fix, changelog, test, and release steps
above. When finished, return to the latest code:
...
This alternative uses one folder and fewer commands. While the release
is in progress, however, your usual project folder and any editable
Python installation based on it will contain the older version.

## Python API

Releases target the exact pushed commit. Automatic changelogs are
limited to the latest release line; maintenance releases use a manually
written changelog.

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L63"
...
*Insert `notes` into changelog `txt` after `marker`, replacing any
existing section for `ver`*

[`update_changelog`](https://nbdev.fast.ai/api/release.html#update_changelog)
makes changelog generation idempotent: if the changelog already has a
section for `ver` (e.g. from an earlier run whose release never
happened), it’s replaced rather than duplicated. That also means
regenerating discards any manual edits to that pending section, so
curate the changelog after its final generation. It also owns the
spacing: two blank lines between version sections and a single newline
at EOF, whether or not `notes` has trailing newlines.
...
Regenerating notes for a version that already has a pending section
gives the same result as generating them fresh:

``` python
upd2 = update_changelog(upd, '0.0.2', '\n## 0.0.2\n\n- Improved feature\n')
test_eq(upd2, update_changelog(log, '0.0.2', '\n## 0.0.2\n\n- Improved feature\n'))
test_eq(upd2.count('## 0.0.2'), 1)
assert 'New feature' not in upd2 and '## 0.0.1' in upd2
```
...
``` python
def Release(
    owner:NoneType=None, repo:NoneType=None, token:NoneType=None, **groups
):
```
...
- **repo:** fetched from the field `name` in `[project]` in
  ...
  You can instead pass `"owner/repo"` (e.g. `"fastai/fastcore"`) to set
  both at once.
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L94"
target="_blank" style="float:right; font-size:smaller">source</a>
...
``` python
rel = Release()
# print(await rel.changelog(debug=True))
```
...
This uses the version information from your `__init__.py`. When
releasing from a branch other than the repo’s default branch (e.g. a fix
to an older version), the release is not marked as the repo’s latest on
GitHub.

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L132"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L141"
target="_blank" style="float:right; font-size:smaller">source</a>
...
``` python
async def changelog(
    debug:<function store_true at 0x7f3b15dfd3a0>=False, # Print info to be added to CHANGELOG, instead of updating file
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L151"
target="_blank" style="float:right; font-size:smaller">source</a>
...
async def release_gh(
    token:str=None, # Optional GitHub token (otherwise `token` file is used)
    repo:str=None, # "repo" or "owner/repo" to use instead of pyproject.toml values
    no_changelog:<function store_true at 0x7f3b15dfd3a0>=False, # Skip changelog creation (assumes CHANGELOG.md is up to date)
    no_editor:<function store_true at 0x7f3b15dfd3a0>=False, # Skip opening CHANGELOG.md in an editor
    yes:<function store_true at 0x7f3b15dfd3a0>=False, # Release without asking for confirmation
):
```

*Create the changelog, optionally edit it, then push and create the
GitHub release*
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L193"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L198"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L204"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L226"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L283"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L289"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L299"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L311"
target="_blank" style="float:right; font-size:smaller">source</a>
...
def release_conda(
    ...
    do_build:<function bool_arg at 0x7f3b15e3d120>=True, # Run `conda build` step
    build_args:str='', # Additional args (as str) to send to `conda build`
    skip_upload:<function store_true at 0x7f3b15dfd3a0>=False, # Skip `anaconda upload` step
    mambabuild:<function store_true at 0x7f3b15dfd3a0>=False, # Use `mambabuild` (requires `boa`)
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L342"
target="_blank" style="float:right; font-size:smaller">source</a>
...
def chk_conda_rel(
    ...
    force:<function store_true at 0x7f3b15dfd3a0>=False, # Always return github tag
):
```
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L357"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L372"
target="_blank" style="float:right; font-size:smaller">source</a>
...
By default, a post-release version increments `.postN`; other versions
increment their patch component. Passing `--part` explicitly selects a
release component and removes the post suffix.

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/nbdev/blob/main/nbdev/release.py#L388"
target="_blank" style="float:right; font-size:smaller">source</a>
