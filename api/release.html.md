...
``` python
async def changelog(
    debug:<function store_true at 0x7f2b03c01300>=False, # Print info to be added to CHANGELOG, instead of updating file
...
async def release_gh(
    token:str=None, # Optional GitHub token (otherwise `token` file is used)
    repo:str=None, # "repo" or "owner/repo" to use instead of pyproject.toml values
    no_changelog:<function store_true at 0x7f2b03c01300>=False, # Skip changelog creation (assumes CHANGELOG.md is up to date)
    no_editor:<function store_true at 0x7f2b03c01300>=False, # Skip opening CHANGELOG.md in an editor
    yes:<function store_true at 0x7f2b03c01300>=False, # Release without asking for confirmation
):
```
...
def release_conda(
    ...
    do_build:<function bool_arg at 0x7f2b03c45120>=True, # Run `conda build` step
    build_args:str='', # Additional args (as str) to send to `conda build`
    skip_upload:<function store_true at 0x7f2b03c01300>=False, # Skip `anaconda upload` step
    mambabuild:<function store_true at 0x7f2b03c01300>=False, # Use `mambabuild` (requires `boa`)
...
def chk_conda_rel(
    ...
    force:<function store_true at 0x7f2b03c01300>=False, # Always return github tag
):
```
