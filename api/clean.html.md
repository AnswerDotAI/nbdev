...
def nbdev_clean(
    ...
    repair:<function bool_arg at 0x7f39ddfd5120>=True, # Fix structural problems, e.g. stray outputs on non-code cells (see `repair_nb`)?
...
def nbdev_install_hooks(
    merge:<function bool_arg at 0x7f39ddfd5120>=True, # Install the notebook merge driver?
    diff:<function bool_arg at 0x7f39ddfd5120>=True, # Install the notebook diff driver?
    globally:<function bool_arg at 0x7f39ddfd5120>=False, # Define the drivers in `~/.gitconfig` and the global attributes file, instead of repo files?
):
```
